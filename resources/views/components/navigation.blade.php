<header
    ...
    <nav :style="scrolled ? null : 'height: 5rem;'" :class="scrolled ? 'h-16 max-w-[92rem] rounded-[1.35rem] bg-white/90 shadow-lg shadow-[#4d4634]/12 ring-1 ring-black/5' : 'max-w-full rounded-none bg-white/96 shadow-sm shadow-[#4d4634]/8 ring-0'"
        ...
        <a class="group flex items-center gap-3 focus:outline-hidden focus:opacity-80" href="{{ url('/') }}" aria-label="Brand">
            ...
            <span class="leading-none">
                <span :class="scrolled ? 'text-xl' : 'text-lg'" class="block font-display font-black uppercase tracking-normal text-[#20221b] transition-all duration-500">{{ config('app.name') }}</span>
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('product-catalog') }}" :class="scrolled ? 'size-11' : 'size-10'" class="hidden items-center justify-center rounded-full bg-[#f2f3ed] text-[#555a42] transition-all duration-500 hover:bg-[#e6e8de] sm:flex" aria-label="Cari produk">
