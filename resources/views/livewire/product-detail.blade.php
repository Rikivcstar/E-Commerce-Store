...
<nav class="product-crumbs" aria-label="Breadcrumb">
            ...
            <div class="product-layout">
                ...
                @endphp
                <section class="product-gallery" aria-label="Product gallery" x-data="{ activeImage: '{{ $coverImg }}' }">
                    <div class="product-main-card">
                        <img :src="activeImage" alt="{{ $product->name }}" class="product-main-image transition-opacity duration-300">

                        <div class="product-gallery-track">
                            <div class="product-thumb cursor-pointer transition-all duration-200"
                                :class="activeImage === '{{ $coverImg }}' ? 'ring-2 ring-[#555a42] opacity-100' : 'opacity-75 hover:opacity-100'"
                                @click="activeImage = '{{ $coverImg }}'">
                                <img src="{{ $coverImg }}" alt="{{ $product->name }} thumbnail">
                            </div>
                            @foreach (collect($galleryImgs)->filter(fn($img) => $img !== $coverImg)->take(2) as $key => $image)
                                <div class="product-thumb cursor-pointer transition-all duration-200"
                                    :class="activeImage === '{{ $image }}' ? 'ring-2 ring-[#555a42] opacity-100' : 'opacity-75 hover:opacity-100'"
