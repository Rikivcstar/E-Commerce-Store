                            ...
                            @if ($this->regions->count() > 0)
                                <ul class="ab-dropdown" x-show="open" x-cloak>
                                    @foreach ($this->regions as $region)
                                        <li wire:key="ab-region-{{ $region->code }}">
                                            <label for="ab-region-{{ $region->code }}" class="w-full inline-block cursor-pointer">
                                                <input type="radio" value="{{ $region->code }}" wire:model.live="region_selector.region_selected"
                                                    class="sr-only" id="ab-region-{{ $region->code }}">
                                                {{ $region->label }}
                                            </label>
                                        </li>
                                    @endforeach
        ...
        @forelse ($addresses as $address)
            <div class="ab-card" data-aos="fade-up">
                ...
                <div class="ab-card-body">
                    <div><strong>{{ $address->full_name }}</strong> &middot; {{ $address->phone }}</div>
