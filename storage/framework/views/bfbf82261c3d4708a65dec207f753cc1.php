...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="mt-5" x-data="{
                    open: false,
                    searchCol: '',
                    collections: <?php echo \Illuminate\Support\Js::from($collections)->toHtml() ?>,
                    get label() {
                    ...
                    <?php $__errorArgs = ['selectCollection.*'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                        ...
                        <div x-show="open" @click.outside="open = false" x-transition.origin.top.duration.150ms
                            ...
                            <div class="max-h-52 overflow-y-auto space-y-1 pr-1">
                                ...
                                <?php $__currentLoopData = $collections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <label x-show="'<?php echo e(strtolower(addslashes($item->name))); ?>'.includes(searchCol.toLowerCase())"
                                        class="flex w-full items-center justify-between rounded-xl px-3 py-2 text-xs font-bold transition text-[#555a42] hover:bg-[#f7f7f2] cursor-pointer">
                                        <div class="flex items-center gap-2 min-w-0">
                                            <input type="checkbox" wire:model.live="selectCollection" value="<?php echo e($item->id); ?>"
                                                class="size-4 rounded-md border-[#c9ccbd] text-[#555a42] focus:ring-[#555a42] checked:bg-[#555a42]">
