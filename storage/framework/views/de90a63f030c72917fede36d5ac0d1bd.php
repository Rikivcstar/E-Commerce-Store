<div>
    <!--[if BLOCK]><![endif]--><?php if($variant === 'icon'): ?>
        <button wire:click.stop.prevent="toggle" type="button"
            class="relative flex size-9 items-center justify-center rounded-full shadow-md backdrop-blur-md transition-all duration-200 hover:scale-110 <?php echo e($isInWishlist ? 'bg-white text-rose-600' : 'bg-white/95 text-zinc-700 hover:text-rose-600'); ?>"
            title="<?php echo e($isInWishlist ? __('Hapus dari Wishlist') : __('Tambah ke Wishlist')); ?>"
            aria-label="Wishlist">
    ...
    <?php else: ?>
        <button wire:click="toggle" type="button"
            class="riva-wish <?php echo e($isInWishlist ? 'riva-wish-active' : 'riva-wish-inactive'); ?>"
            aria-label="Wishlist">
            ...
            <span>
                <?php echo e($isInWishlist ? __('Disimpan') : __('Simpan')); ?>
